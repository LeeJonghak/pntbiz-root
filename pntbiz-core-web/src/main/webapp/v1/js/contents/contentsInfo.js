$(document).ready( function() {
	...
	$('#typeNum').bind('change', function() { contents.setForm($('#typeNum').val()) });
...
var contents = {
	...
	_compURL : "/contents/info/component.ajax.do",
	_listURL: "/contents/info/list.do",
	_mappingDataURL: "/contents/mappingData.ajax.do",
	_mappingURL: "/contents/mapping.ajax.do",
	_formURL: "/contents/info/form.do",
	_mformURL: "/contents/info/mform.do",
	_regURL: "/contents/info/reg.do",
	_modURL: "/contents/info/mod.do",
	_delURL: "/contents/info/del.do",
	_fileDelURL: "/contents/info/fileDel.do",
	_form: "#form1",
	...
	initForm: function(list) {
		for(var i=0; i<list.length; i++) {
			console.log(list[i]);
		}
	},
	setForm: function(typeNum) {
		$.ajax({ type: "POST",
			contentType: 'application/x-www-form-urlencoded',
			processData: false,
			url: this._compURL,
			data: $(this._form).serialize(),
			success: this.setFormResult
		});
	},
	setFormResult: function(data) {
		var result = {};
		if(typeof(data)=='string') {
			result = $.parseJSON(data);
		} else {
		...
		switch(result.result) {
			case "1" :
				contents.initForm(result.list);
				break;
			case "2" : common.error(vm.infoFail); break;
			default : common.error(vm.infoError); break;
		}
	},
	form: function() {
