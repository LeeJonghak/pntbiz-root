 ...
 *   provides complete control over the message shown to the end admin.user, including
...
class Validate {
	...
	/**
	 * Extend the options from the admin.user function and the validation function
	 * with core defaults.
	 *
	 *  @internal
	 */
	public static function _extend( $userOpts, $prop, $fnOpts ) {
		...
		// key clash, so admin.user first, then function commands and finally the
		// global options 
	...
	static function basic( $val, $data, $opts, $host ) {
		// Common validation with admin.user override option
	...
	static function notEmpty( $val, $data, $opts, $host ) {
		...
		// non-empty (admin.user can override by passing in `empty=true` in opts
