$(document).ready( function() {
  if(window.location.pathname=="/contents/form.do")
    contents.initForm();
  else if(window.location.pathname=="/contents/mform.do")
    contents.setForm($("#conType").val());
  if($('#contentsKeyword').val() == "") $('#contentsKeyword').focus();
	$('#contentsKeyword').bind('keyup', function(e) { if(e.keyCode==13) contents.search(); });
	$('#checkAll_contents').bind('click', function() { common.checkboxId('#checkAll_contents','checkbox'); });
	$("#checkAll_target").bind('click', function () { common.checkboxId('#checkAll_target','checkbox'); });
...
var contents = {
	...
	multselectReset: function(optionText) {
		$("#targetlist").html(optionText);

/*
		var data = "<li><label class='checkBox'><input type='radio' name='a'  value='target' class='noneSelected'> None Selected</label></li>"
				+ "<li><label class='checkBox'><input type='radio' name='a' value='target' class='allSelected'> All Selected</label></li>"
				+ "<li class='title'>Reference List</li>"+optionText;

		$("#multiselect4").html(data);
*/
...
/*
			if(mappingType == 'GF' || mappingType == 'GFG') {
				$("#selectUl_fcEventType .selected").removeClass("selected");
				$("#selectUl_fcEventType a:eq(0)").addClass("selected");
				$('#selectUl_fcEventType').parent().show();
			}else {
				$('#selectUl_fcEventType').parent().hide();
			}
*/
      $.ajax({ type: "POST",
...
*/
		...
		switch(result.result) {
			case "1" :
				...
				for (var i = 0; i < _list.length; i++) {
					...
					optionText = optionText + "<li><label class='checkBox'><input type='checkbox' value='"+_list[i].refNum+"' class='targetCheckbox'>"+_list[i].refName+"</label></li>";
				}
	...
	mapping: function() {
		...
		var refNums = $("#targetlist .targetCheckbox:checked");			// 매핑 대상 번호
		var mappingType = $("#selectBoxMapping option:selected").val();	// 매핑유형
		var evnetNum = $("#multiselect1 .selected").attr("data-value");	// 이벤트
	...
	mappingResult: function(data) {
		...
		switch(result.result) {
			case "1" :
				...
				/*
                  $(".aright .selectUlWrap").each(function () {
                    var selectName = $(this).find("ul").attr("data-name");
                    $("select[name="+selectName+"]").val($(this).find("a").eq(0).attr("data-value"));
                    $(this).find("b").text($(this).find("a").eq(0).html());
                    $(this).find("a").removeClass("selected");
                    $(this).find("a").eq(0).addClass("selected");
                  });
                  $(".aright select[name=fcEventType]").children().first().prop("checked", true);
                $(".aright select[name=fcEventType]").css("display", "none");
        */
