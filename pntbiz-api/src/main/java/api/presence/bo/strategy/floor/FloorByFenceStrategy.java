 ...
 */
public class FloorByFenceStrategy extends AbstractPresenceStrategy {
    ...
    protected void doStrategy(PresenceDataPackage presenceVo) {
        ...
        if (isSameFloor == true) {                          // 1.       현재상태와 이전상태 층이 같을 경우
            ...
            if (
                    ...
                    previousInOutStateType == STAY ||         // 2.1      이전상태가 IN, STAY 일 경우 OUT 설정
                    previousInOutStateType == STAYING
                )
                setPreviousFloorOut();
    ...
    private void setFloorStay() {
        ...
        if (floorInOutState.getStayTimeInterval() > 0) {
            Integer baseTime = (floorInOutState.getStayTime() == 0) ? floorInOutState.getInTime() : floorInOutState.getStayTime();
            if (createdTimeStamp > baseTime + floorInOutState.getStayTimeInterval()) {
                currentPresenceRedis.getFloorInOutState().setState(STAY);
                currentPresenceRedis.getFloorInOutState().setStayTime(createdTimeStamp);
            }
        } else {
            currentPresenceRedis.getFloorInOutState().setState(STAY);
            currentPresenceRedis.getFloorInOutState().setStayTime(createdTimeStamp);
    ...
    private void setFloorStaying() {
        currentPresenceRedis.setFloorInOutState(previousPresenceRedis.getFloorInOutState());
        currentPresenceRedis.getFloorInOutState().setState(STAYING);
    }
    ...
    private void setFloorIn() {
        ...
        floorState.setInTime(createdTimeStamp);
        floorState.setStayTimeInterval(60 * 10);
