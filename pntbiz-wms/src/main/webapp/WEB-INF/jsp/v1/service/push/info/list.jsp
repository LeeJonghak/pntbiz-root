	...
	<script>
		$(function(){
			$(".pushSendBtn").click(function(){
				var _pnsStageSeqNo = $(this).data('pnsstageseqno');
				window.location.href='<c:url value="/service/push/info/form.do"/>?pnsStageSeqNo=' + _pnsStageSeqNo;
...
<body>
	...
	<div class="col-sm-6">
		<h3>푸쉬발송대기목록 <small></small></h3>
	</div>
	<div class="col-sm-6">
		<span class="pull-right">
			<ol class="breadcrumb">
			  <li><a href="#">서비스</a></li>
			  <li class="active">푸쉬발송대기목록</li>
			</ol>
		</span>
	</div>
	<hr />
				...
				<c:otherwise>
					<c:forEach var="item" items="${list}" varStatus="status">
						<tr align="center">
							...
							<td>
								<c:choose>
									<c:when test="${item.sendType eq 'AP' }">
										<button type="button" class="btn btn-default pushSendBtn" data-pnsstageseqno="${item.pnsStageSeqNo }" >발송하기</button>	
									</c:when>
									<c:otherwise>
										&nbsp;
									</c:otherwise>
